var target_admin = function () {
    ...
    var layoutColors = ['#16a085', '#e74c3c', '#428bca', '#333333', '#6685a4',
        '#E68E8E']
    ...
    function initTableCheckable() {
        if ($.fn.tableCheckable) {
            $('.table-checkable').tableCheckable().on('masterChecked',
                    function (event, master, slaves) {
                        if ($.fn.iCheck) {
                    ...
                    .each(
                            function () {
                                ...
                                var defaultOptions = {
                                    ...
                                    lengthChange: false,
                                    displayRows: 10
                                }, dataOptions = $(this).data(), helperOptions = $
                                        .extend(defaultOptions, dataOptions), $thisTable, tableConfig = {}
                                ...
                                if (!helperOptions.search) {
                                    $thisTable.parent().find(
                                            '.dataTables_filter').remove()
                                }

                                var filterableCols = $thisTable
                                        .find('thead th').filter(
                                        '[data-filterable="true"]')

                                if (filterableCols.length > 0) {
                                    var columns = $thisTable.fnSettings().aoColumns, $row, th, $col, showFilter
                                    ...
                                    for (var i = 0; i < columns.length; i++) {
                                        ...
                                        showFilter = ($col.data('filterable') === true) ? 'show'
                                                : 'hide'
                                    ...
                                    $row
                                            .find('th')
                                            .removeClass(
                                                    'sorting sorting_disabled sorting_asc sorting_desc sorting_asc_disabled sorting_desc_disabled')
                                    ...
                                    $thisTable
                                            .addClass('datatable-columnfilter')
                                }
                            })
